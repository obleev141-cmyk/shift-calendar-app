...
# Интерфейс Streamlit (чтобы страница не была пустой)
st.title("Telegram Shift Bot Server")
st.write("Статус: Бот запущен и работает в фоне.")
...
def create_calendar_visual(surname, raw_line):
    ...
    for r_idx, week in enumerate(cal_structure):
        for c_idx, day_num in enumerate(week):
            ...
            # ВЫХОДНОЙ - ЗЕЛЕНЫЙ, РАБОТА - ОРАНЖЕВЫЙ
            bg_color = (60, 140, 85) 
            shift_info = ""
...
async def get_ocr_result(image_bytes, surname):
    try:
        ...
        return f"Фамилия {surname} не найдена."
...
async def handle_photo(message: types.Message):
    ...
    wait_msg = await message.answer("⏳ Обработка...")
...
# Запуск бота в отдельном потоке для Streamlit
def run_bot():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    # Используем skip_updates, чтобы избежать конфликтов при перезагрузке Streamlit
